...
if [[ "${DEPLOY_ONLY}" != "true" ]]; then
  ...
  if [[ ! -f "${root_schema}" ]]; then
    echo "Error: Prisma schema not found: ${root_schema}" >&2
    exit 1
  fi

  for schema_copy in "${schema_copies[@]}"; do
    if [[ ! -f "${schema_copy}" ]]; then
      echo "Error: Prisma schema not found: ${schema_copy}" >&2
      exit 1
    fi
    if ! cmp -s "${root_schema}" "${schema_copy}"; then
      echo "Error: Prisma schema mismatch: ${schema_copy} differs from ${root_schema}" >&2
      exit 1
    fi
  done

  dockerfile_path="${REPO_ROOT}/docker/Dockerfile.non_root"
  if [[ ! -f "${dockerfile_path}" ]] || ! grep -Fxq 'ARG PROXY_EXTRAS_SOURCE=local' "${dockerfile_path}"; then
    echo "Error: Gomo release requires docker/Dockerfile.non_root to install local proxy extras" >&2
    exit 1
  fi
fi
