...
client = openai.OpenAI(
	api_key="${_||"YOUR_LITELLM_API_KEY"}",
	base_url="${v}"
)`;switch(h){case i.CHAT:{let e=Object.keys(k).length>0,r="";if(e){let e=JSON.stringify({metadata:k},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();r=`,
...
`;break}case i.RESPONSES:{let e=Object.keys(k).length>0,r="";if(e){let e=JSON.stringify({metadata:k},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();r=`,
    extra_body=${e}`}let a=C.length>0?C:[{role:"user",content:j}];t=`
import base64
