...
def make_ui_spend_logs_mock_prisma(mock_spend_logs, filter_fn, team_lookup_fn=None):
    ...
    class MockDB:
        ...
        async def query_raw(self, sql_query, *params):
            ...
            if "CASE WHEN session_id IS NULL OR session_id = ''" in sql_query:
                grouped = {
                    (f"session:{log['session_id']}" if log.get("session_id") else f"request:{log['request_id']}"): log
                    for log in reversed(filtered)
                }
                filtered = list(reversed(tuple(grouped.values())))
            total = len(filtered)
