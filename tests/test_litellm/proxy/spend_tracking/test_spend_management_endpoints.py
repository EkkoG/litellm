...
@pytest.mark.asyncio
async def test_build_ui_spend_logs_response_enriches_user_alias():
    class FakeUserTable:
...
@pytest.mark.asyncio
async def test_build_ui_spend_logs_response_session_rows_include_user_aliases():
    class FakeUserTable:
        async def find_many(self, where=None, **kwargs):
    ...
    session_rows = [{"row_type": "session", "users": ["u1", "u2"]}]
    ...
    assert result["data"][0]["user_aliases"] == (
...
@pytest.mark.asyncio
async def test_build_ui_spend_logs_response_user_alias_lookup_failure_is_safe():
