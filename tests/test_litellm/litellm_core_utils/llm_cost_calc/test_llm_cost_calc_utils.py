...
def test_chatgpt_codex_auto_review_pricing() -> None:
    repository_root = os.path.abspath(os.path.join(os.path.dirname(litellm.__file__), ".."))
    main_price_map_path = os.path.join(repository_root, "model_prices_and_context_window.json")
    backup_price_map_path = os.path.join(
        os.path.dirname(litellm.__file__),
        "model_prices_and_context_window_backup.json",
    )

    with open(main_price_map_path) as main_price_map_file, open(backup_price_map_path) as backup_price_map_file:
        main_price_map = json.load(main_price_map_file)
        backup_price_map = json.load(backup_price_map_file)
    ...
    assert main_price_map[model] == backup_price_map[model]
    assert {key: main_price_map[model][key] for key in expected_pricing} == expected_pricing
