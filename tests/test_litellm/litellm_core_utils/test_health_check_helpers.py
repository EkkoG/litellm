...
@pytest.mark.asyncio
async def test_ahealth_check_does_not_hydrate_copilot_credential_in_display_params():
    captured_litellm_params = None
    model_params = {
        "model": "github_copilot/claude-opus-4.8",
        "litellm_credential_name": "copilot-credential",
    }

    def fake_get_llm_provider(*args, **kwargs):
        nonlocal captured_litellm_params
        captured_litellm_params = kwargs.get("litellm_params")
        return "claude-opus-4.8", "github_copilot", None, None

    async def fake_health_check_handler():
        return MagicMock(_hidden_params={"headers": {}})

    with (
        patch("litellm.main.get_llm_provider", side_effect=fake_get_llm_provider),
        patch.object(
            HealthCheckHelpers,
            "get_mode_handlers",
            return_value={"chat": fake_health_check_handler},
        ),
    ):
    ...
    assert captured_litellm_params is None
    assert "github_copilot_access_token" not in model_params
