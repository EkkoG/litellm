...
def _valid_auth_record() -> dict[str, object]:
    return {
...
def test_parse_xai_auth_json_accepts_cli_record():
    result = parse_xai_auth_json(json.dumps(_valid_auth_record()))

    assert result.access_token == "access-token"
    assert result.refresh_token == "refresh-token"
    assert result.token_endpoint == "https://auth.x.ai/oauth/token"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"expires_at": True},
        {"expires_at": 0},
        {"token_endpoint": "http://auth.x.ai/oauth/token"},
        {"token_endpoint": "https://attacker.example/oauth/token"},
        {"token_endpoint": "https://user@auth.x.ai/oauth/token"},
        {"token_endpoint": "https://auth.x.ai/oauth/token#fragment"},
        {"unexpected": "value"},
    ],
)
def test_parse_xai_auth_json_rejects_invalid_records(patch):
    payload = {**_valid_auth_record(), **patch}

    with pytest.raises(XAIOAuthError, match="Invalid xAI OAuth credential JSON"):
        parse_xai_auth_json(json.dumps(payload))


@pytest.mark.parametrize("raw", ["[]", '"token"', "null", "{not-json"])
def test_parse_xai_auth_json_requires_json_object(raw):
    with pytest.raises(XAIOAuthError, match="Invalid xAI OAuth credential JSON"):
        parse_xai_auth_json(raw)


@pytest.mark.asyncio
async def test_async_refresh_auth_record_preserves_refresh_token():
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        refreshed = await XAIOAuthAuthenticator().async_refresh_auth_record(
            XAIAuthRecord.model_validate(_valid_auth_record()),
            http_client=client,
        )
