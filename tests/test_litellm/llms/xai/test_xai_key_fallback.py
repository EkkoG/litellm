...
def test_get_api_key_legacy_flag_does_not_expose_generic_key(monkeypatch):
    monkeypatch.setattr(litellm, "xai_key", None)
    monkeypatch.setattr(litellm, "api_key", "common_api_key")
    monkeypatch.setenv("XAI_API_KEY", "env_api_key")

    assert XAIModelInfo.get_api_key(None, legacy_generic_before_env=True) == "env_api_key"
...
def test_responses_config_does_not_expose_generic_key(monkeypatch):
    monkeypatch.setattr(litellm, "xai_key", None)
    monkeypatch.setattr(litellm, "api_key", "common_api_key")
    monkeypatch.setenv("XAI_API_KEY", "env_api_key")
...
def test_responses_config_raises_when_no_key_is_available(monkeypatch):
    ...
    assert "litellm.api_key" not in error_message
...
def test_realtime_config_does_not_expose_generic_key_when_provider_does_not_resolve_key(
    monkeypatch,
):
    captured_kwargs = {}
